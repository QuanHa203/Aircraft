...
{    
    ...
    while (1)
    {
        ...
        while (1)
        {
            ...
            else
            {
                ...
                ESP_LOGI("LOG", "Received: \"%.*s\"\n", payload_len, tcp_receive_buffer);

                bool ok = rf_tx->transmit(tcp_receive_buffer, payload_len, 10);
                if (ok)
                    printf("[TX] Gửi thành công!\n");
                else
    ...
    while (1)
    {
        ...
        // bool ok = rf_tx.transmit(tx_payload, payload_len, 100);
